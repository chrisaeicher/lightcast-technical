...
export default function OccupationSummary({ summaryObj, jobTitle }) {
	...
	const [isPositiveGrowth, setIsPositiveRegionalGrowth] = useState(null);
	...
	const getRegAndNatComparison = () => {
		const percentChange = (
			(summaryObj.jobs.regional / summaryObj.jobs.national_avg) *
			100
		).toFixed(0);
	...
	return (
		<div>
			...
			<div className="grid grid-rows-3 divide-y sm:grid-rows-1 sm:grid-cols-3 sm:divide-x border-gray-300 border-t-2 border-b-2">
				<TotalJobsCard
					regionalJobs={regionalJobs}
					natAvgComparison={natAvgComparison}
				...
				<PercentChangeCard
					startYear={startYear}
					endYear={endYear}
					regional={regionalGrowth}
					nationalGrowth={nationalGrowth}
				/>
				<HourlyEarningsCard
					regionalAvgEarnings={regionalAvgEarnings}
